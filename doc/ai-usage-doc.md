...
Used CoPilot to solve indentation error. Comments are put in locations where they were used.

ChatGPT recommended to use different files for the functions that are required to be implemented.
...
With the aid of Claude and CoPilot, I made corrections to the issues that were listed in `grade.txt`. 
`Height` and `size` were fixed in `shapes.scala` file.